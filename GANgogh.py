...
# --- TensorFlow compatibility: run TF1-style graph code even on TF2 ---
import tensorflow as _tf
tf = _tf.compat.v1 if hasattr(_tf, "compat") else _tf
if hasattr(tf, "disable_v2_behavior"):
    tf.disable_v2_behavior()

# Ensure local imports (tflib) work when running from repo root
...
class Config:
    ...
    classes: int = 14
    preiterations: int = 2000
    out_dir: str = "generated"

def apply_config(cfg: Config) -> None:
    """Populate legacy global constants from Config (minimal-change refactor)."""
    globals().update({
        "MODE": cfg.mode,
        "DIM": cfg.dim,
        ...
        "ITERS": cfg.iters,
        "LAMBDA": cfg.lambda_gp,
        "CLASSES": cfg.classes,
        "PREITERATIONS": cfg.preiterations,
    })
    globals()["OUTPUT_DIM"] = DIM * DIM * 3
    globals()["DEVICES"] = ["/gpu:{}".format(i) for i in range(N_GPUS)]
    os.makedirs(cfg.out_dir, exist_ok=True)

def parse_args() -> Config:
    p = argparse.ArgumentParser(description="Train / smoke-test AC-WGAN-GP on WikiArt genres.")
    p.add_argument("--iters", type=int, default=Config.iters, help="Training iterations (default: 200000). Use small for smoke test.")
    p.add_argument("--preiters", type=int, default=Config.preiterations, help="Pretraining iterations for classifier head.")
    p.add_argument("--batch_size", type=int, default=Config.batch_size, help="Batch size.")
    p.add_argument("--critic_iters", type=int, default=Config.critic_iters, help="Critic updates per generator update.")
    p.add_argument("--lambda_gp", type=float, default=Config.lambda_gp, help="Gradient penalty weight (lambda).")
    p.add_argument("--classes", type=int, default=Config.classes, help="Number of genre classes.")
    p.add_argument("--out_dir", type=str, default=Config.out_dir, help="Output directory for samples/checkpoints.")
    p.add_argument("--mode", type=str, default=Config.mode, choices=["acwgan", "wgan-gp"], help="Training mode.")
    args = p.parse_args()
    return Config(
        mode=args.mode,
        dim=Config.dim,
...
def run_training(cfg: Config) -> None:
    """Build the graph and run training. Defaults can be reduced for a smoke test."""
    apply_config(cfg)

    # The original code writes outputs to a hard-coded "generated/" directory.
    # We keep that for compatibility and also create your chosen output directory.
